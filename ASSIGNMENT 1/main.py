...
products = [
    ...
    {'id': 5, 'name': 'keyboard',       'price': 599, 'category': 'Electronics', 'in_stock': True },
    {'id': 6, 'name': 'pencil',       'price':  30,  'category': 'Stationery',  'in_stock': True },
    {'id': 7, 'name': 'USB cable',         'price': 199, 'category': 'Electronics', 'in_stock': False},
]
...
def get_instock(): 
    ...
    if not available:
        return {"error": "No products in stock"}
...
def get_deals(): 
    cheapest = min(products, key=lambda p: 
    p["price"]) 
